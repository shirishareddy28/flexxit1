...
import { getMovieVideoSource } from "../../../Data/hooks/movieSlice";
import {
  setUserProfile,
  updateUserProfileLikes,
  updateUserProfileDislikes,
  updateUserProfileMylist,
  getMyListMovies,
} from "../../../Data/hooks/UserSlice";
import { useLoader } from "../../../Data/hooks/UseLoader";

export const MovieModal = ({ item, handleClose }) => {
  const [mute, setMute] = useState(true);
  const { movieDetail, movieVideoSource } = useSelector(
    (state) => state.movies
  );
  ...
  const isLiked = (id) => {
    return userProfile.likes && userProfile.likes.length > 0
      ? userProfile.likes.includes(id)
      : false;
  };

  // check movie is disliked or not
  const isDisLiked = (id) => {
    return userProfile.dislikes && userProfile.dislikes.length > 0
      ? userProfile.dislikes.includes(id)
      : false;
  };

  // check movie is added to mylist or not
  const isInList = (id) => {
    return userProfile.myList && userProfile.myList.length > 0
      ? userProfile.myList.includes(id)
      : false;
  };
  ...
  const handleLike = async (movieId) => {
    ...
    let isExist =
      tempLikes && tempLikes.filter((x) => x === movieId).length > 0
        ? true
        : false;
  ...
  const handleDislike = async (movieId) => {
    ...
    let isDisLikeExist =
      tempDislikes && tempDislikes.filter((x) => x === movieId).length > 0
        ? true
        : false;
    let filteredDisLikes = tempDislikes
      ? tempDislikes.filter((x) => x !== movieId)
      : [];
    let newDislikes = !isDisLikeExist
      ? [...tempDislikes, movieId]
      : [...filteredDisLikes];
  ...
  const handleAddToList = async (movieId) => {
    ...
    let isMylistExist =
      tempMylist && tempMylist.filter((x) => x === movieId).length > 0
        ? true
        : false;
    let filteredMylist = tempMylist
      ? tempMylist.filter((x) => x !== movieId)
      : [];
    let newMylist = !isMylistExist
      ? [...tempMylist, movieId]
      : [...filteredMylist];
    obj.likes = [...userProfile.likes];
    obj.dislikes = [...userProfile.dislikes];
    ...
    <>
      <Box className="reactplayer">
        <iframe
          ...
          title="hello"
          height="576"
          src={`https://youtube.com/embed/${movieVideoSource.key}?autoplay=1&showinfo=0&controls=0&mute=${mute}&rel=0`}
          frameBorder="0"
          autoPlay="1"
          ...
          <div className="synopsis">
            ...
            <div className="synopsis-tooltips">
              {inList ? (
                ...
                >
                  <AddIcon
                    onClick={() => handleAddToList(movieDetail.id)}
                    className="highlight"
                    style={{ fontSize: "24px" }}
                  />
                </Tooltip>
              ...
              {liked ? (
                ...
                >
                  <ThumbUpAltOutlinedIcon
                    onClick={() => handleLike(movieDetail.id)}
                    className="highlight"
                    style={{ fontSize: "24px" }}
                  />
                </Tooltip>
              ...
              {disliked ? (
                ...
                >
                  <ThumbDownAltOutlinedIcon
                    onClick={() => handleDislike(movieDetail.id)}
                    className="highlight"
                    style={{ fontSize: "24px" }}
                  />
                </Tooltip>
        ...
        <div className="movie-info">
          <div className="movie-info-left">
            <div className="movie-info-left_-first">
              <p className="movie-info-left-first_green">{`${
            ...
            <div>
              ...
              {movieDetail.genres ? (
                ...
                movieDetail.genres.map((genre, i) => {
                  return (
                    <span className="movie_info_right_links" key={genre.id}>
                      {genre.name}
                      {`${movieDetail.genres.length - 1 !== i ? "," : ""}`}
                    </span>
                  );
                })
              ) : (
